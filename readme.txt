...
git-tx synchronizes a directory in your local repository to a corresponding directory in a remote git repository. It supports composing projects from all or part of other projects.  Unlike git submodules or 'fake' git submodules,  with git transplant you issue explicit commands to control synchronization.  Normal git commands in a git-tx tracked directory work as if there was no tx tracking.  Clones of the super-project are normal repos. Only the developers who need to update the tracking directories need to know about git transplant.
